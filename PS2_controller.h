...
#define SERVOMIN 204 // This is the 'minimum' pulse length count (out of 4096)
...
bool motor3_enable = false;
bool motor4_enable = false;
...
bool PS2control()
{
  ...
  int nJoyX = X_JOY_CALIB - ps2x.Analog(PSS_LX); // read x-joystick
  int nJoyY = Y_JOY_CALIB - (driving_mode ? ps2x.Analog(PSS_RY) :ps2x.Analog(PSS_LY)); // read y-joystick
  ...
  {
    c1 = nMotMixL;
    c1 = map(c1, 0, 128, 0, 3800);
  }
  else if (nMotMixL < 0)
  {
    c2 = abs(nMotMixL);
    c2 = map(c2, 0, 128, 0, 3800);
  }
  setPWMMotors_backward(c1, c2, c3, c4);

  // Phan dieu khien 2 dong co DC phia sau
  if (ps2x.ButtonPressed(PSB_R1)){
    motor3_enable = !motor3_enable;
  }
  
  if (ps2x.ButtonPressed(PSB_R2)){
    motor4_enable = !motor4_enable;
  }

  setPWMMotors_remain(motor3_enable, motor4_enable, TOP_SPEED, NORM_SPEED);

  // Phan dieu khien dong cơ servo
  if (ps2x.Button(PSB_GREEN)) // Servo 180
  {
    servo_enable = !servo_enable;
  }

  setPWMMotors_servo(servo_enable, SERVOMAX, SERVOMIN);
